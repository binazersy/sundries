...
int main(int argc, const char **argv)
{
    ...
    Couchbase::Client h(connstr, "password", "Administrator");
    Status rv = h.connect();
    if (!rv.success()) {
	...
	auto my_val{ R"({ "vga":"2070 super", "cpu":"Ryzen 3600" })"s };
	auto mres = h.upsert("myval", my_val);
	cout << "Got status for store. Cas=" << std::hex << mres.cas() << endl;
